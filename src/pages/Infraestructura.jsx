        ...
        <section className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-5 mb-8 hover:shadow-md transition-shadow duration-300">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
            ...
            <div className="relative">
              ...
              <button
                type="button"
                onClick={() => setEstadoAbierto(!estadoAbierto)}
                className="w-full flex items-center justify-between gap-2 px-3 py-2.5 border border-gray-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-[#701330]/30 focus:border-[#701330] transition-all text-sm"
              >
                <span className="flex items-center gap-2">
                  <span
                    className="w-2.5 h-2.5 rounded-full flex-shrink-0"
                    style={{ backgroundColor: filtroEstado === 'todos' ? COLORS.gray : getEstiloEstado(filtroEstado).color }}
                  />
                  <span className="text-gray-800">{filtroEstado === 'todos' ? 'Todos' : filtroEstado}</span>
                </span>
                <svg
                  className={`w-4 h-4 text-gray-400 transition-transform duration-200 ${estadoAbierto ? 'rotate-180' : ''}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              {estadoAbierto && (
                <>
                  <div className="fixed inset-0 z-30" onClick={() => setEstadoAbierto(false)} />
                  <div className="absolute z-40 w-full mt-1.5 bg-white rounded-xl border border-gray-200 shadow-xl overflow-hidden">
                    {[
                      ...
                      const color = op.valor === 'todos' ? COLORS.gray : getEstiloEstado(op.valor).color;
                      return (
                        <button
                          key={op.valor}
                          type="button"
                          onClick={() => { setFiltroEstado(op.valor); setEstadoAbierto(false); }}
                          ...
                          style={{
                            ...
                            backgroundColor: activo ? `${color}0F` : undefined
                          }}
                        >
