...
const Iconos = {
  ...
  historial: (
    <svg className="w-5 h-5 transition-transform duration-300 group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 14h6M9 17h4" />
    </svg>
  ),
  chevron: (
    ...
    <>
      ...
      `}>
        ...
        <nav className="flex flex-col gap-1.5 mt-4 px-3 flex-1 relative z-10 overflow-y-auto sidebar-scroll">
          ...
          <ItemLink to="/historial" icon={Iconos.historial}>
            Historial
          </ItemLink>
          ...
          {esSuperAdmin && (
            <>
              ...
              <ItemLink to="/carreras" icon={Iconos.carreras}>
                Carreras
              </ItemLink>
          ...
          <div className="px-3">
            <button
              ...
              className="w-full flex items-center justify-between gap-2 px-3 py-2 rounded-lg bg-white/10 hover:bg-white/20 transition-colors text-sm text-white"
              title="Prender o apagar el modo noche"
            >
            ...
            <span className={`relative inline-flex h-5 w-9 shrink-0 items-center rounded-full transition-colors duration-200 ${tema === 'oscuro' ? 'bg-[#701330]' : 'bg-white/25'}`}>
              <span className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white shadow transition-transform duration-200 ${tema === 'oscuro' ? 'translate-x-[18px]' : 'translate-x-0.5'}`}></span>
            </span>
            </button>
          </div>
